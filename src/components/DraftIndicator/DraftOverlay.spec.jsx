...
describe("DraftOverlay", () => {
  ...
  describe("when there is a fetch error", () => {
    it("should render the error state", () => {
      ...
      expect(
        getByText(
          "An error occurred while loading your drafts. Please try again."
        )
      ).toBeTruthy();
    });

    it("should not render the empty state when there is a fetch error", () => {
      ...
      expect(queryByText("No drafts yet")).toBeNull();
    });
  });
  ...
  describe("when there are drafts", () => {
    it("should render all draft rows", () => {
      const { getByText } = renderWithIntl(
        <DraftOverlay
          formDrafts={mockFormDraftsWithData}
          onClose={mockOnClose}
          onSelect={mockOnSelect}
        />
      );
    ...
    it("should call onSelect with the correct draft when a row is clicked", () => {
      ...
      const firstRowButton = container.querySelectorAll(
        ".ipd-draft-overlay__row-button"
      )[0];
    ...
    it("should call onSelect with correct draft for second row click", () => {
      ...
      const secondRowButton = container.querySelectorAll(
        ".ipd-draft-overlay__row-button"
      )[1];
